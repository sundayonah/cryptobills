...
export function AirtimeSwapCard() {
  ...
  const onSubmit = async (data: AirtimeFormData) => {
    ...
    try {
      ...
      // Ensure user is ready and authenticated
      if (!ready || !authenticated) {
        throw new Error("Please wait for authentication to complete.");
      }

      // Check if user has embedded wallet (required for Privy's native gas sponsorship)
      const hasEmbeddedWallet = wallets?.some(w => 
        w.connectorType === 'embedded' || w.walletClientType === 'privy'
      );
      
      if (!hasEmbeddedWallet) {
        throw new Error("Gas sponsorship requires an embedded wallet. Please sign in with email or social login instead of an external wallet.");
      }
      ...
      // Switch chain if needed (for embedded wallets)
      if (wallets && wallets.length > 0) {
        const wallet = wallets[0];
      ...
      // Use Privy's native gas sponsorship with embedded wallets (TEE)
      // This works with useSendTransaction + sponsor: true per Privy docs
      const txHashResponse = await sendTransaction(
        {
          to: tokenConfig.address,
          data: transferData,
          value: BigInt(0),
          chainId, // Important: specify the chain
        },
        {
      ...
      // sendTransaction returns an object with hash property
      const txHash = typeof txHashResponse === 'string' 
        ? txHashResponse 
